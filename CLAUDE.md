...
A Gmail-triage agent that classifies each email (newsletter / receipt / calendar / personal / work / banking / application / assessment / junk) and decides on a label/archive/draft action. Personal-only v1, **live in draft phase** — real emails come in via Gmail Push → Pub/Sub → webhook, get classified, labeled, and (for personal/work/application) have a reply draft generated.
