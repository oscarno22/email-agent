...
- **`classify`** (`core/classifier.py`): forced tool use on `classify_email`. **Two-tier cost strategy**: `Tier.FAST` first; if `confidence < 0.6`, escalate to `Tier.DEEP` and mark `needs_escalation=True`. Tiers map to concrete models inside `core/llm.py` (`FAST`→Haiku/Gemini Flash-Lite, `DEEP`→Sonnet/Gemini Flash-Lite). The category enum exposed to the model excludes `UNKNOWN` — `UNKNOWN` is reserved for code paths the model can't pick.
- **LLM provider abstraction** (`core/llm.py`): both `classify` and `generate_draft` go through `get_provider()`, which returns a `Provider` (`structured_completion` for forced-tool output, `text_completion` for prose). The factory composes providers from env keys: both `ANTHROPIC_API_KEY` + `GEMINI_API_KEY` present → `FallbackProvider(AnthropicProvider, GeminiProvider)` (Anthropic primary, Gemini auto-fallback on any `LLMError`); only one key → that provider alone; neither → raises `LLMError` at first use. Gemini uses `gemini-3.1-flash-lite` for both tiers via the `google-genai` SDK; its JSON-schema response uses the same `CLASSIFY_TOOL` schema (numeric `minimum`/`maximum` keys are stripped — Gemini's response_schema is an OpenAPI-3 subset). Thinking is pinned to `thinking_level=MINIMAL`: Gemini 3 has no thinking-off switch and **rejects the old `thinking_budget` outright**, so the 2.5-era `thinking_budget=0` is gone. Because MINIMAL still emits some thought tokens and they count against `max_output_tokens` (a truncated response arrives as empty text → `LLMError`), the Gemini paths carry extra output headroom — 1024 for classification, `max(caller, 1024)` for drafts. The factory is `@lru_cache(maxsize=1)`; tests `patch("agent.core.classifier.get_provider")`.
